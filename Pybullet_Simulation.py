...
class Simulation(Simulation_base):
    ...
    def jacobianMatrix(self, endEffector):
        ...
        for joint in self.functionJoints(endEffector):
            ...
            jacobianMatrix = np.append(jacobianMatrix,np.array(np.cross(self.jointRotationAxis[joint],(self.getJointPosition(endEffector)-self.getJointPosition(joint)))).T)
